...
export class ElectricianService {
    ...
    async submitCannotAttend(electricianId: number, complaintId: number, reason?: string) {
        ...
        if (!['assigned', 'in_progress'].includes(c.status)) {
            throw new BadRequestException(`Cannot decline complaint in status "${c.status}"`)
        }
        return this.prisma.complaint.update({
            where: { id: complaintId },
            data: {
    ...
    async submitResolution(
        ...
        if (input.localJobId) {
            ...
            if (prior) {
                if (prior.id !== complaintId) {
                    throw new BadRequestException('localJobId already used for another complaint')
                }
        ...
        if (!c.pole_id || !c.pole) {
            throw new BadRequestException('Complaint has no pole — cannot validate proof location')
        }
