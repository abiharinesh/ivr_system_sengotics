...
export class VoiceToTextService {
  ...
  constructor(
    ...
    this.genAI = new GoogleGenerativeAI(geminiKey || 'dummy-key');

    if (!groqKey)
      this.logger.warn(
        'GROQ_API_KEY not set — GROQ transcription will not work',
      );
    if (!geminiKey)
      this.logger.warn(
        'GEMINI_API_KEY not set — Gemini transcription will not work',
      );
  }
  ...
  async transcribeAudio(audioUrl: string, retryCount = 0): Promise<string> {
    ...
    this.logger.log(
      `[STT:${provider}] Transcribing audio from: ${audioUrl}${retryCount > 0 ? ` (retry #${retryCount})` : ''}`,
    );
    ...
    try {
      ...
      if (this.isHallucination(text)) {
        ...
        this.logger.warn(
          'Hallucination persists after retry — returning empty',
        );
        return '';
      }
      ...
      if (isRetryable && retryCount < 1) {
        const delay = 3000;
        this.logger.warn(
          `Transcription failed (${errMessage}). Retrying in ${delay}ms...`,
        );
  ...
  private async transcribeWithGroq(audioBuffer: ArrayBuffer): Promise<string> {
    ...
    const timeout = new Promise<any>((_, reject) =>
      setTimeout(() => reject(new Error('Groq STT timed out')), 25000),
    );
  ...
  private async transcribeWithGemini(
    ...
    const mimeType = audioUrl.includes('.wav')
      ? 'audio/wav'
...
- Preserve the exact words spoken, including Tanglish (Tamil-English mix)`,
    ...
    const timeout = new Promise<any>((_, reject) =>
      setTimeout(() => reject(new Error('Gemini STT timed out')), 25000),
    );
  ...
  private async transcribeWithRapidAPI(
    audioBuffer: ArrayBuffer,
  ): Promise<string> {
    ...
    if (!rapidApiKey) {
      throw new Error(
        'RapidAPI key not configured. Set RAPIDAPI_KEY in .env or via PUT /api/superadmin/settings/api-keys',
      );
    }
  ...
  private async transcribeWithGoogleSpeech(
    ...
    if (!googleKey) {
      this.logger.warn(
        'GOOGLE_SPEECH_API_KEY not configured. Falling back to Gemini.',
      );
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          config: {
            encoding: audioUrl.includes('.wav')
              ? 'LINEAR16'
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Google Speech Error - ${response.status}: ${errorText}`,
        );
      }
  ...
  private async downloadAudio(
    ...
    if (exotelApiKey && exotelApiToken && audioUrl.includes('exotel')) {
      ...
      if (retryCount === 0)
        this.logger.warn(
          'EXOTEL_API_KEY or EXOTEL_API_TOKEN not set — fetching without auth (may fail)',
        );
    }
  ...
  private isHallucination(text: string): boolean {
    ...
    const hasTamilKeywords =
      /kovil|koil|koyil|pakkam|kitta|maram|kadai|theru|veedu|school|bus|stand|hospital|temple|mosque|church|light|pole|current|wire/i.test(
        lower,
      );
