...
export class VoiceProcessingService {
    ...
    async processPhase1Voice(callSid: string, audioUrl: string, ivrNumber: string): Promise<VoiceProcessingResult> {
        ...
        try {
            const result = await this.runPhase1Pipeline(reg.voiceCallId, callSid, audioUrl, ivrNumber)
            ...
            return { success: false, status: 'not_found', attemptNumber: reg.attemptNumber, phase: 1, message: 'Phase1 failed' }
        }
    }

    async acceptPhase2Llm(callSid: string, audioUrl: string, ivrNumber: string): Promise<VoiceProcessingResult> {
        ...
        this.runPhase2EnrichmentOnly(reg.voiceCallId, callSid, audioUrl, ivrNumber).catch(async (err) => {
            this.logger.error(`[Phase2] Background failure for ${callSid}: ${(err as Error).message}`)
            await this.safeUpdateStatus(reg.voiceCallId, 'failed')
            await this.prisma.callState.update({
                where: { call_sid: callSid },
    ...
    async processPendingPhase2Llm(limit = 10): Promise<{ scanned: number, processed: number, skipped: number }> {
        const states = await this.prisma.callState.findMany({
            where: { phase2_status: { in: ['queued', 'processing'] }, phase2_voice_call_id: { not: null } },
        ...
        for (const state of states) {
            ...
            const phase2Row = await this.prisma.voiceCall.findUnique({
                ...
                select: { id: true, audio_url: true, processing_status: true, call_sid: true }
            })
            ...
            if (['completed', 'manual_review', 'superseded'].includes(phase2Row.processing_status)) {
                skipped++
                continue
            }
            ...
            try {
                await this.runPhase2EnrichmentOnly(phase2Row.id, state.call_sid, phase2Row.audio_url, ivrNumber)
                processed++
            } catch {
                skipped++
            }
    ...
    private async runPhase1Pipeline(
        ...
        if (!transcript?.trim()) {
            ...
            return { success: false, status: 'not_found', message: 'Phase1 empty transcript' }
        }
        ...
        await this.prisma.voiceCall.update({
            where: { id: voiceCallId },
            data: { ai_extracted_json: extracted as any, confidence_score: 1, processing_status: 'completed' }
        })
        ...
        if (!complaint) {
            return { success: true, status: 'completed', message: 'Complaint already finalized for call' }
        }

        this.enrichInBackground(complaint.id, voiceCallId, transcriptEnglish, panchayat.id).catch(() => { })
    ...
    private async runPhase2EnrichmentOnly(
        ...
        const state = await this.prisma.callState.findUnique({ where: { call_sid: callSid } })
        if (!state) return

        if (state.complaint_created && state.complaint_id) {
            if (transcriptEnglish.trim()) {
                await this.enrichExistingComplaintWithPhase2(state.complaint_id, transcript, transcriptEnglish)
            }
        ...
        const phase1Data = await this.waitForPhase1VoiceCall(state.phase1_voice_call_id, callSid)
        const panchayat = await this.geoMatching.findPanchayatByIvrNumber(ivrNumber)

        if (!panchayat) {
            await this.createManualReviewComplaint(callSid, voiceCallId, audioUrl, undefined, transcript, transcriptEnglish)
            return
        }

        const combinedText = [phase1Data?.transcript, phase1Data?.transcript_english, transcript, transcriptEnglish]
            .filter(Boolean)
            .join('. ')
            .trim()

        if (!combinedText) {
            await this.createManualReviewComplaint(callSid, voiceCallId, audioUrl, panchayat.id, transcript, transcriptEnglish)
            return
        }
        ...
        const hints = [extracted.landmark_english, extracted.landmark].filter(Boolean) as string[]
        const poleId = await this.geoMatching.findNearestPole(panchayat.id, hints)
        if (!poleId) {
            await this.createManualReviewComplaint(callSid, voiceCallId, audioUrl, panchayat.id, transcript, transcriptEnglish)
            return
        }

        await this.finalizeComplaintFromPhase2(callSid, voiceCallId, poleId, panchayat.id, extracted, audioUrl)
    }
    ...
    private async registerPhaseAttempt(
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (duplicate) {
                return { voiceCallId: duplicate.id, attemptNumber: duplicate.attempt_number, isDuplicate: true }
            }
            ...
            await tx.callState.upsert({
                ...
                update: phase === 'phase1'
                    ? { phase1_status: 'processing', phase1_voice_call_id: voiceCall.id }
    ...
    private async finalizeComplaintFromPhase2(
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (state?.complaint_created) {
                await tx.voiceCall.update({ where: { id: voiceCallId }, data: { processing_status: 'completed' } })
                await tx.callState.update({ where: { call_sid: callSid }, data: { phase2_status: 'enriched' } })
                return
            }
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const complaint = await tx.complaint.create({
                data: {
                    ...
                    complaint_type: 'street_light',
                    description: transcriptEnglish || transcript || 'Auto flagged for review',
    ...
    private async waitForPhase1VoiceCall(phase1VoiceCallId: number | null, callSid: string) {
        ...
        while (Date.now() - start < PHASE1_WAIT_MAX_MS) {
            const row = phase1VoiceCallId
                ? await this.prisma.voiceCall.findUnique({
                    ...
                    select: { transcript: true, transcript_english: true, processing_status: true }
                })
    ...
    private async enrichExistingComplaintWithPhase2(
        ...
        if (!current?.description || current.description.length < transcriptEnglish.length) {
            await this.prisma.complaint.update({
                where: { id: complaintId },
                data: { description: transcriptEnglish || transcript || current?.description || undefined }
            })
        }
    }
    ...
    private async safeUpdateStatus(voiceCallId: number, status: string): Promise<void> {
        try {
            await this.prisma.voiceCall.update({ where: { id: voiceCallId }, data: { processing_status: status } })
        } catch (err) {
            this.logger.error(`[DB] Failed to update VoiceCall #${voiceCallId}: ${(err as Error).message}`)
        }
    }
