...
const EXTRACTION_PROMPT = `You extract location + complaint details from Colloquial Tamil/Tanglish/English voice transcripts about infrastructure issues (electrical and water supply) in Tamil Nadu villages.
...
Return JSON only:
{
  ...
  "complaint_type": "<light pole not working|power cut|wire damage|transformer issue|water_leak|water_supply_shortage|other>",
...
Output: {"village":"","landmark":"Mariamman kovil pakkathula","landmark_english":"near Mariamman temple","direction":"near","complaint_type":"light pole not working","confidence_score":0.9,"call_summary":"The street light near Mariamman temple is not working."}

2. Clear landmark (Tanglish mix):
Input: "bus stand kitta current poguthu"
Output: {"village":"","landmark":"bus stand kitta","landmark_english":"near bus stand","direction":"near","complaint_type":"power cut","confidence_score":0.85,"call_summary":"There is a power cut near the bus stand."}

3. Water Leak (Tamil):
Input: "Annur kovil pakkathula kulai odஞ்சு thanni oluguthu"
Output: {"village":"","landmark":"Annur kovil pakkathula","landmark_english":"near Annur temple","direction":"near","complaint_type":"water_leak","confidence_score":0.92,"call_summary":"Water is leaking from a broken pipe near the Annur temple.","caller_language":"colloquial/spokentamil","caller_emotion":"panicked","urgency_level":"high"}

4. Water Shortage (Tamil):
Input: "Water Tank Lane-la rendu naala thanni varala"
Output: {"village":"","landmark":"Water Tank Lane-la","landmark_english":"at Water Tank Lane","direction":"inside","complaint_type":"water_supply_shortage","confidence_score":0.88,"call_summary":"No water supply has been received at Water Tank Lane for two days.","caller_language":"tanglish","caller_emotion":"frustrated","urgency_level":"medium"}
...
RULES:
- Speech-to-Text AIs (like Whisper, Google STT, or RapidAPI) often make spelling mistakes in Tamil script (e.g. "ஆரியப்பத் கோவிலிட்ட இங்கிரலையிட்" actually means "மாரியம்மன் கோயில் பக்கத்துல" -> "near Mariamman temple"). You MUST sound out the Tamil words phonetically and map them to logical infrastructure landmarks.
...
export class LocationExtractionService {
  ...
  async extractLocation(
    ...
    if (knownLandmarks?.length) {
      this.logger.log(
        `Known landmarks provided: [${knownLandmarks.join(', ')}]`,
      );
    }
  ...
  private async extractWithGemini(userMessage: string): Promise<string> {
    ...
    const timeout = new Promise<any>((_, reject) =>
      setTimeout(() => reject(new Error('Gemini extraction timed out')), 15000),
    );
