...
export class ZoneService {
    ...
    async update(panchayatId: number, zoneId: number, dto: UpdateZoneDto) {
        ...
        if (!existing) {
            throw new NotFoundException(`Zone #${zoneId} not found in panchayat #${panchayatId}`)
        }
        ...
        return this.prisma.panchayatZone.update({
            ...
            data: {
                ...
                ...(dto.boundary_geojson != null && { boundary_geojson: dto.boundary_geojson as any }),
     ...
     * typically does not.
     */
    async lookupBoundary(placeName: string, countryCode = 'in'): Promise<NominatimResult[]> {
        ...
        return results.filter(r => r.geojson && (r.geojson.type === 'Polygon' || r.geojson.type === 'MultiPolygon'))
    }
    ...
    private validateGeoJson(geojson: Record<string, unknown>) {
        if (!geojson || typeof geojson !== 'object') {
            throw new BadRequestException('boundary_geojson must be a valid GeoJSON object')
        }

        const type = geojson.type as string
        if (type !== 'Polygon' && type !== 'MultiPolygon' && type !== 'Feature' && type !== 'FeatureCollection') {
        ...
        if ((type === 'Polygon' || type === 'MultiPolygon') && !geojson.coordinates) {
            throw new BadRequestException('Polygon/MultiPolygon must have coordinates')
        }
    }
}
