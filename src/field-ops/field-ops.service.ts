...
export class ElectricianOpsService {
    ...
    private async ensureElectricianInPanchayat(electricianId: number, panchayatId: number) {
        const u = await this.prisma.user.findUnique({ where: { id: electricianId } })
        if (!u || u.role !== 'electrician') throw new BadRequestException('Not an electrician')
        if (u.panchayat_id !== panchayatId) throw new ForbiddenException('Electrician is in another panchayat')
        return u
    }
    ...
    async createResolvedExportJob(args: {
        ...
        if (args.scopedPanchayatId != null && u.panchayat_id !== args.scopedPanchayatId) {
            throw new ForbiddenException('Electrician belongs to another panchayat')
        }
            ...
            for (const c of complaints) {
                ...
                lines.push(
                    [
                        ...
                        c.resolution_location_valid === null ? '' : String(c.resolution_location_valid),
                        csvEscape(diskName),
                    ].join(',')
                )
            }
