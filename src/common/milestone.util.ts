...
export const DEFAULT_MILESTONE_RULES: MilestoneRule[] = [
  ...
  },
  {
    id: 'award_date',
    label: 'Award date',
    source: 'milestone:comparative_due',
    offset_days: 0,
  },
  {
    id: 'completion_deadline',
    label: 'Completion deadline',
    source: 'milestone:award_date',
    offset_days: 3,
  },
  {
    id: 'so_proceedings_target',
    label: 'SO proceedings target',
    source: 'work_completed_at',
    offset_days: 14,
  },
  {
    id: 'voucher_target',
    label: 'Voucher target',
    source: 'milestone:so_proceedings_target',
    offset_days: 7,
  },
];
...
export function resolveMilestones(
  ...
  for (const r of rules) {
    if (
      dates[r.id] == null &&
      r.source !== 'work_completed_at' &&
      r.source !== 'work_order_date' &&
      r.source !== 'anchor'
    ) {
      // Likely a cycle or missing referenced milestone.
      const refId =
        typeof r.source === 'string' && r.source.startsWith('milestone:')
