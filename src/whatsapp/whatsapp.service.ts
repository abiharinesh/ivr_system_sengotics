...
export class WhatsAppService {
    ...
    verifyWebhookToken(token: string | undefined): boolean {
        ...
        if (!expected) {
            this.logger.warn('WHATSAPP_VERIFY_TOKEN not set — webhook verification fails')
            return false
        }
    ...
    verifySignature(rawBody: Buffer, signatureHeader: string | undefined): boolean {
        ...
        if (!secret) {
            this.logger.warn('WHATSAPP_APP_SECRET not set — accepting webhook without signature check (dev only)')
            return true
        }
