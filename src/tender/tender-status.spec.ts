import { assertTransition, validateAccessMode, validateTemplateId } from './tender-status'
...
describe('tender status machine', () => {
    ...
    it('walks the canonical happy path', () => {
        expect(() => assertTransition('draft', 'published')).not.toThrow()
        expect(() => assertTransition('published', 'quotations_closed')).not.toThrow()
        expect(() => assertTransition('quotations_closed', 'vendor_selected')).not.toThrow()
        expect(() => assertTransition('vendor_selected', 'field_verification')).not.toThrow()
        expect(() => assertTransition('field_verification', 'closed')).not.toThrow()
    })
})
...
describe('validateTemplateId', () => {
    it('accepts the six known templates', () => {
        const ids = ['rfq', 'quotation', 'comparative', 'work_order', 'so_proceedings', 'form19']
        for (const id of ids) {
