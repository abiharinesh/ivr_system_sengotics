...
import { sanitizeOverlaySvg, sanitizeTemplateDefaults } from './overlay-sanitize'

describe('overlay-sanitize', () => {
    it('accepts minimal svg', () => {
        const out = sanitizeOverlaySvg('<svg xmlns="http://www.w3.org/2000/svg"><rect width="10" height="10"/></svg>')
        expect(out).toContain('<svg')
    })

    it('rejects script tags', () => {
        expect(() => sanitizeOverlaySvg('<svg><script>alert(1)</script></svg>')).toThrow(BadRequestException)
    })
