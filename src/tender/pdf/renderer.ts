...
function isVercelRuntime(): boolean {
  return (
    Boolean(process.env.VERCEL) || process.env.AWS_LAMBDA_FUNCTION_NAME != null
  );
}
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(
              new Error(
                `Gotenberg HTTP ${res.statusCode}: ${body.toString('utf8').slice(0, 300)}`,
              ),
            );
          }
        });
...
async function renderWithGotenberg(html: string): Promise<Buffer | null> {
  ...
  try {
    ...
    const pdfBuf = await postToGotenberg(
      endpoint,
      form,
...
async function launchBrowser(): Promise<any | null> {
  ...
  if (isVercelRuntime()) {
    ...
    if (!chromium) {
      logger.warn(
        '@sparticuz/chromium not installed; PDF generation unavailable on Vercel',
      );
      return null;
    }
    try {
