...
import { DOCUMENT_TEMPLATE_IDS, DocumentTemplateId, validateTemplateId } from '../tender-status'
...
import { sanitizeFabricScene, sanitizeOverlaySvg, sanitizeTemplateDefaults } from './overlay-sanitize'
...
export class DocumentTemplateSettingsService {
    ...
    private async loadPanchayatMap(panchayatId: number): Promise<TemplateSettingsMap> {
    ...
    async updateGlobalSettings(patch: { templates?: unknown }) {
        ...
        const row = await this.prisma.systemSettings.upsert({
            ...
            create: { key: DOCUMENT_TEMPLATE_DEFAULTS_KEY, value: JSON.stringify(merged) },
        })
    ...
    async updatePanchayatSettings(panchayatId: number, patch: { templates?: unknown }) {
        if (!patch?.templates) {
            throw new BadRequestException('templates object is required')
        }
        const sanitized = sanitizeSettingsMap(patch.templates)
        ...
        return {
            scope: 'panchayat' as const,
            panchayat_id: panchayatId,
            templates: this.packAllTemplates(global, parsePanchayatSettingsJson(p.document_template_settings)),
            updated_at: null,
        }
    }
    ...
    async savePanchayatDesign(
        ...
        const global = await this.loadGlobalMap()
        this.logger.log(`Document template design updated (panchayat ${panchayatId}): ${tpl}`)
        return {
            scope: 'panchayat' as const,
            panchayat_id: panchayatId,
            template_id: tpl,
    ...
    private designPatchForTemplate(
        ...
        const baseDefaults = { ...((current?.defaults ?? {}) as Record<string, unknown>) }
    ...
    async resetPanchayatTemplate(panchayatId: number, templateId: string) {
        ...
        await this.prisma.panchayat.update({
            where: { id: panchayatId },
            data: { document_template_settings: Object.keys(existing).length ? (existing as any) : null },
        })
        return this.getPanchayatSettings(panchayatId)
    }

    async resolveMergedLayout(panchayatId: number, templateId: DocumentTemplateId): Promise<TemplateLayoutConfig> {
    ...
    async resolveMergedFieldOverrides(
        ...
        const defaults = (layout.defaults ?? {}) as Record<string, unknown>
    ...
    async buildPreviewHtml(
        ...
        const ctx =
            options?.tender_id != null
                ...
                : await this.buildSampleContext(panchayatId, tpl, options?.vendor_id ?? null)
    ...
    private packAllTemplates(global: TemplateSettingsMap, panchayat: TemplateSettingsMap) {
        return DOCUMENT_TEMPLATE_IDS.map((id) => {
            const effective = getMergedLayout(id, global, panchayat)
            const hasPanchayatOverride = Boolean(panchayat[id] && Object.keys(panchayat[id]!).length)
            const hasGlobalOverride = Boolean(global[id] && Object.keys(global[id]!).length)
            return {
                template_id: id,
    ...
    private async buildSampleContext(
        ...
        const panchayat = await this.prisma.panchayat.findUnique({ where: { id: panchayatId } })
        if (!panchayat) throw new NotFoundException(`Panchayat #${panchayatId} not found`)

        const layout = await this.resolveMergedLayout(panchayatId, templateId)
        const fieldOverrides = mergeFieldOverrides((layout.defaults ?? {}) as Record<string, unknown>)
        ...
        return {
            ...
            invited_vendors: [
                { id: 1, name: 'Sample Vendor', phone_e164: '+919876543210', place: 'Town' },
            ],
            quotations: [
                {
                    id: 1,
