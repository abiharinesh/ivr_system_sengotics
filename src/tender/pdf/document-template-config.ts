...
export interface TemplateLayoutConfig {
    ...
    table?: { headerAlign?: TextAlign; bodyAlign?: TextAlign; numberAlign?: 'left' | 'right' }
...
export type TemplateSettingsMap = Partial<Record<DocumentTemplateId, TemplateLayoutConfig>>
...
export function builtinDefaultFor(templateId: DocumentTemplateId): TemplateLayoutConfig {
    return deepMergeConfig(BUILTIN_DEFAULT, {}) as TemplateLayoutConfig
}

export function builtinDefaultsMap(): Record<DocumentTemplateId, TemplateLayoutConfig> {
...
export function mergeFieldOverrides(
    ...
    for (const part of parts) {
        ...
        for (const [k, v] of Object.entries(part)) {
            if (k === '__canvas_layers' && Array.isArray(v) && Array.isArray(result.__canvas_layers)) {
...
const SIG_ALIGNMENTS: SignatureAlign[] = ['left', 'center', 'right', 'justify', 'space-between']

function parseAlign(v: unknown, allowed: readonly string[], fallback: string): string {
...
export function sanitizeTemplateConfig(input: unknown): TemplateLayoutConfig {
    ...
    if (isPlainObject(input.header)) {
        out.header = { align: parseAlign(input.header.align, ALIGNMENTS, 'center') as TextAlign }
    }
    if (isPlainObject(input.body)) {
        out.body = { align: parseAlign(input.body.align, ALIGNMENTS, 'left') as TextAlign }
    }
    if (isPlainObject(input.table)) {
        out.table = {
            headerAlign: parseAlign(input.table.headerAlign, ALIGNMENTS, 'left') as TextAlign,
            bodyAlign: parseAlign(input.table.bodyAlign, ALIGNMENTS, 'left') as TextAlign,
            numberAlign: input.table.numberAlign === 'left' ? 'left' : 'right',
        }
    }
    if (isPlainObject(input.signatures)) {
        out.signatures = {
            align: parseAlign(input.signatures.align, SIG_ALIGNMENTS, 'center') as SignatureAlign,
        }
    }
...
export function sanitizeSettingsMap(input: unknown): TemplateSettingsMap {
    if (!isPlainObject(input)) {
        throw new BadRequestException('templates must be an object keyed by template id')
    }
...
export function parseSettingsJson(raw: string | null | undefined): TemplateSettingsMap {
...
export function layoutStyleBlock(layout: TemplateLayoutConfig): string {
    ...
    const sigAlign = sig.align === 'space-between' ? 'space-between' : sig.align ?? 'center'
    const sigTextAlign = sig.align === 'space-between' ? 'center' : sig.align ?? 'center'
    return `
  :root {
...
export const TEMPLATE_LABELS: Record<DocumentTemplateId, { ta: string; en: string }> = {
