...
export class PanchayatDocumentTemplateSettingsController {
    ...
    private getPanchayatId(req: AuthenticatedRequest): number {
        if (!req.user.panchayat_id) {
            throw new ForbiddenException('Account is not associated with a panchayat')
        }
    ...
    @Put()
    updateSettings(@Req() req: AuthenticatedRequest, @Body() body: { templates?: unknown }) {
        return this.settings.updatePanchayatSettings(this.getPanchayatId(req), body)
    }

    @Delete(':templateId')
    resetTemplate(@Req() req: AuthenticatedRequest, @Param('templateId') templateId: string) {
        validateTemplateId(templateId)
        return this.settings.resetPanchayatTemplate(this.getPanchayatId(req), templateId)
    }
    ...
    saveDesign(
        ...
        validateTemplateId(templateId)
        return this.settings.savePanchayatDesign(this.getPanchayatId(req), templateId, body)
    }
