...
export class TenderPdfService {
  ...
  private buildStoragePath(subdir: string, filename: string): string {
    return path.posix
      .join(subdir.replace(/\\/g, '/'), filename)
      .replace(/\\/g, '/');
  }
  ...
  private async resolveQuotationVendorId(
    tenderId: number,
  ): Promise<number | null> {
  ...
  private normalizedCanvasLayers(
    input: unknown,
    ...
    return input
      ...
      .map((item: any, idx) => ({
        ...
        page: Number.isFinite(Number(item.page))
          ? Math.max(1, Math.floor(Number(item.page)))
          : 1,
  ...
  private async renderAndPersistArtifacts(doc: {
    ...
    await this.documentStorage.writeBuffer(
      pdfStoragePath,
      pdfBuf,
      'application/pdf',
    );
  ...
  private async ensureArtifactExists(doc: {
    ...
    if (await this.documentStorage.exists(doc.storage_path))
    ...
    this.logger.log(
      `Re-generating artifact for doc ${doc.id} (storage_path missing from disk/bucket)`,
    );
    try {
      const storagePath = await this.renderAndPersistArtifacts(doc);
      await this.prisma.tenderDocument.update({
  ...
  async resolveDocStoragePath(
    ...
    if (!doc || doc.tender_id !== tenderId)
      throw new NotFoundException('Document not on tender');
    if (doc.status !== 'ready' || !doc.storage_path)
      throw new BadRequestException('Document not ready');
    ...
    if (format === 'pdf') {
      ...
      if (storagePath.toLowerCase().endsWith('.pdf')) {
        try {
          ...
          /* fall through to on-the-fly conversion */
        }
      }
...
${bodyContent}
  ...
  async getCanvasState(panchayatId: number, tenderId: number, docId: number) {
    ...
    return {
      ...
      version: doc.version,
      can_edit: canEdit,
      locked_reason: canEdit
        ? null
        : 'Canvas editing is locked for this document template',
      layers,
    };
  }
