...
export function sanitizeOverlaySvg(raw: unknown): string | null {
  ...
  if (s.length > MAX_OVERLAY_SVG_CHARS) {
    throw new BadRequestException(
      `overlay_svg exceeds ${MAX_OVERLAY_SVG_CHARS} characters`,
    );
  }
  const lower = s.toLowerCase();
  if (!lower.includes('<svg')) {
    throw new BadRequestException(
      'overlay_svg must contain an SVG root element',
    );
  }
  if (/<script\b/i.test(s)) {
    throw new BadRequestException(
      'overlay_svg must not contain script elements',
    );
  }
  if (/\bon\w+\s*=/i.test(s)) {
    throw new BadRequestException(
      'overlay_svg must not contain event handlers',
    );
  }
...
export function sanitizeFabricScene(
  raw: unknown,
  ...
  if (typeof raw === 'string') {
    if (raw.length > MAX_FABRIC_SCENE_JSON_CHARS) {
      throw new BadRequestException(
        `fabric_scene exceeds ${MAX_FABRIC_SCENE_JSON_CHARS} characters`,
      );
    }
    try {
      obj = JSON.parse(raw);
...
export function sanitizeTemplateDefaults(
  raw: unknown,
