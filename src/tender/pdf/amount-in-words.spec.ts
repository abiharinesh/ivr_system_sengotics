...
describe('rupeesInWordsEn', () => {
    ...
    it('handles the Sundaramoorthy fixture amount', () => {
        ...
        expect(rupeesInWordsEn(6500).toLowerCase())
            .toContain('six thousand five hundred')
        expect(rupeesInWordsEn(6500).toLowerCase()).toContain('rupees only')
    })

    it('handles lakhs', () => {
        expect(rupeesInWordsEn(125000).toLowerCase())
            .toContain('one lakh twenty-five thousand')
    })

    it('handles crores', () => {
        expect(rupeesInWordsEn(15000000).toLowerCase()).toContain('one crore fifty lakh')
    })
})
