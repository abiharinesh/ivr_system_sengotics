...
export function parseLatLongFromText(text: string): { lat: number | null; lng: number | null } {
    const normalized = text.replace(/°/g, ' ').replace(/[\u2018\u2019\u201C\u201D]/g, "'")

    const patterns = [
        new RegExp(`Lat\\s*${COORD_NUMBER}\\s*[,\\s]*Long\\s*${COORD_NUMBER}`, 'i'),
        new RegExp(`latitude[:\\s]+${COORD_NUMBER}.*longitude[:\\s]+${COORD_NUMBER}`, 'is'),
...
export type CoordSource = 'exif' | 'ocr_overlay' | 'exif_and_ocr' | 'manual' | null
...
export function fuseCoordinates(args: {
    ...
    if (args.manualPoleId) {
        return { lat: args.exifLat ?? args.ocrLat, lng: args.exifLng ?? args.ocrLng, coordSource: 'manual' }
    }
    ...
    if (hasExif && hasOcr) {
        const drift = haversineM(args.exifLat!, args.exifLng!, args.ocrLat!, args.ocrLng!)
        if (drift <= 50) {
            return { lat: args.exifLat, lng: args.exifLng, coordSource: 'exif_and_ocr' }
        }
        return { lat: args.ocrLat, lng: args.ocrLng, coordSource: 'ocr_overlay' }
    }
    if (hasExif) return { lat: args.exifLat, lng: args.exifLng, coordSource: 'exif' }
    if (hasOcr) return { lat: args.ocrLat, lng: args.ocrLng, coordSource: 'ocr_overlay' }
...
export function normalizePoleToken(value: string): string {
    return value.trim().toUpperCase().replace(/^POLE\s*/i, '').replace(/^P[-#]?\s*/i, '').replace(/\s+/g, '')
}

export function matchPoleFromOcrText(
    text: string,
    candidates: Array<{ id: number; pole_number: string | null; keypad_id: string | null }>,
): { poleId: number | null; poleNumber: string | null; keypadId: string | null; confidence: 'single' | 'ambiguous' | 'unmatched' | 'skipped' } {
    if (!text.trim() || candidates.length === 0) {
        return { poleId: null, poleNumber: null, keypadId: null, confidence: 'skipped' }
    }
    ...
    if (unique.length > 1) {
        return { poleId: null, poleNumber: null, keypadId: null, confidence: 'ambiguous' }
    }
    return { poleId: null, poleNumber: null, keypadId: null, confidence: 'unmatched' }
}
