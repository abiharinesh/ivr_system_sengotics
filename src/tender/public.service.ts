...
export class TenderPublicService {
  ...
  private ensureDeadlineOpen(
    ...
    if (Date.now() > d.getTime()) {
      throw new BadRequestException(
        `Quotation deadline passed for tender #${tenderId}`,
      );
    }
  }
  ...
  private ensureWritableStatus(status: string) {
    if (status !== 'published') {
      throw new BadRequestException(
        `Tender is not accepting quotations (status=${status})`,
      );
    }
  }

  private async enforceOpenAbuseGuards(
    tenderId: number,
    ipAddress: string,
    normalizedPhone: string,
  ) {
    const ipHash = createHash('sha256')
      .update(`${ipAddress ?? ''}|${tenderId}`)
      .digest('hex')
      .slice(0, 24);
    ...
    if (ipRecent >= OPEN_IP_LIMIT) {
      throw new BadRequestException(
        'Too many submissions from this network. Please retry later.',
      );
    }
    if (phoneRecent >= OPEN_PHONE_LIMIT) {
      const bucket = phoneHash(normalizedPhone);
      throw new BadRequestException(
        `Too many submissions from this phone (${bucket}). Please retry later.`,
      );
    }
  }
  ...
  async submitInviteQuotation(args: {
    ...
    if (t.quotation_access_mode !== 'invited_only') {
      throw new BadRequestException(
        'This invite is not valid for open tenders',
      );
    }
    this.ensureWritableStatus(t.status);
    const timeline = await this.milestones.resolveForTender(t.id);
