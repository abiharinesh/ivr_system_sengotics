...
export class FieldVerificationController {
    ...
    listSessions(@Req() req: AuthenticatedRequest, @Param('id', ParseIntPipe) id: number) {
        return this.service.listSessions(this.getPanchayatId(req), id)
    }

    @Get(':id/checklist')
    getChecklist(@Req() req: AuthenticatedRequest, @Param('id', ParseIntPipe) id: number) {
    ...
    patchChecklist(
        ...
        @Body() body: { is_done?: boolean; verified_upload_id?: number | null; notes?: string | null }
    ) {
    ...
    confirm(@Req() req: AuthenticatedRequest, @Param('id', ParseIntPipe) id: number) {
        return this.service.confirmVerification(this.getPanchayatId(req), id, req.user.id)
    }
}
