...
export class FieldVerificationService {
  ...
  async createSession(
    ...
    if (body.pole_subset_ids !== undefined && subset.length === 0) {
      throw new BadRequestException(
        'pole_subset_ids must include at least one pole',
      );
    }
    if (!subset.length) {
    ...
    if (!subset.length) {
      throw new BadRequestException(
        'No poles linked to this tender; add line items with poles first',
      );
    }
    ...
    const expiresAt = new Date(
      Date.now() + (body.expires_in_days ?? 14) * 24 * 60 * 60 * 1000,
    );
  ...
  async patchChecklistItem(
    ...
    if (!item || item.tender_id !== tenderId) {
      throw new NotFoundException(
        `Checklist item #${itemId} not on this tender`,
      );
    }
    ...
    if (body.verified_upload_id !== undefined) {
      if (body.verified_upload_id != null) {
        ...
        if (!upload || upload.session.tender_id !== tenderId) {
          throw new BadRequestException(
            'verified_upload_id not on this tender',
          );
        }
      }
  ...
  async assignUpload(
    ...
    if (subset.length > 0 && !subset.includes(poleId)) {
      throw new BadRequestException(
        `Pole #${poleId} is not in this upload section`,
      );
    }
  ...
  async confirmVerification(
    ...
    if (items.length === 0) {
      throw new BadRequestException(
        'No checklist items exist; create a verification session first',
      );
    }
    const incomplete = items.filter((it) => !it.is_done);
    if (incomplete.length > 0) {
      throw new BadRequestException(
        `Checklist incomplete: ${incomplete.length} item(s) not done`,
      );
    }
    ...
    if (t.auto_resolve_linked_complaints) {
      ...
      for (const li of lineItems) {
        ...
        const itm =
          items.find((i) => i.line_item_id === li.id) ??
          items.find((i) => i.pole_id === li.pole_id);
        ...
        if (proof) {
          ...
          data.resolution_image_latitude =
            proof.matched_lat ?? proof.exif_lat ?? proof.ocr_lat;
          data.resolution_image_longitude =
            proof.matched_lng ?? proof.exif_lng ?? proof.ocr_lng;
          data.resolution_image_captured_at =
            proof.captured_at ?? proof.ocr_captured_at;
        ...
        try {
          await this.prisma.complaint.update({
            where: { id: li.complaint_id },
            data,
          });
          resolvedCount++;
        } catch (err: unknown) {
          this.logger.warn(
            `Failed to bulk-resolve complaint #${li.complaint_id}: ${err instanceof Error ? err.message : err}`,
          );
        }
      }
    }
  ...
  private matchNearestPole(
    ...
    const distances = candidates
      ...
      .map((p) => ({
        id: p.id,
        d: distanceMeters(lat, lng, p.latitude!, p.longitude!),
      }))
      .sort((a, b) => a.d - b.d);
  ...
  private buildUploadMessage(args: {
    ...
    if (
      args.confidence === 'single_nearest' &&
      poleId &&
      args.distanceMeters != null
    ) {
  ...
  async submitUpload(args: {
    ...
    const exifPromise = (async () => {
      ...
      try {
        const exif = await exifr.parse(args.image.buffer, {
          gps: true,
    ...
    const ocrPromise = Promise.race([
      ...
      new Promise<
        Awaited<ReturnType<typeof this.overlayOcr.extractOverlayCoords>>
      >((resolve) =>
        setTimeout(() => {
    ...
    const polesPromise: Promise<PoleCandidate[]> = session.pole_subset_ids
      .length
      ? this.prisma.electricPole.findMany({
    ...
    if (args.manualPoleId) {
      ...
      const gpsMatch = this.matchNearestPole(
        fused.lat,
        fused.lng,
        candidatePoles,
        radiusM,
