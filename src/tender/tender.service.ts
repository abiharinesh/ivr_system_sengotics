...
export class TenderService {
  ...
  async list(orgUnitId?: number, opts: { status?: string } = {}) {
    ...
    const fvIds = tenders
      .filter((t) => t.status === 'field_verification')
      .map((t) => t.id);
    ...
    if (fvIds.length > 0) {
      ...
      const doneMap = new Map(
        doneRows.map((r) => [r.tender_id, r._count._all]),
      );
    ...
    return tenders.map((t) => ({
      ...
      verification_progress:
        t.status === 'field_verification'
          ? (progressByTender.get(t.id) ?? { done: 0, total: 0 })
          : null,
    }));
  }
  ...
  async create(
    ...
    const accessMode: QuotationAccessMode = validateAccessMode(
      body.quotation_access_mode ?? 'invited_only',
    );
    const anchor = body.anchor_date ? new Date(body.anchor_date) : null;
    if (anchor && Number.isNaN(anchor.getTime())) {
      throw new BadRequestException(
        'anchor_date must be a valid ISO 8601 date',
      );
    }
    const rules =
      body.milestone_rules && body.milestone_rules.length > 0
  ...
  async patch(
    ...
    const nonAnchorKeys = Object.keys(body).filter(
      (k) =>
        k !== 'anchor_date' && body[k as keyof TenderCreateBody] !== undefined,
    );
    if (
      nonAnchorKeys.length > 0 &&
      existing.status !== 'draft' &&
      existing.status !== 'published'
    ) {
      throw new BadRequestException(
        'Tender details can only be edited while draft or published',
      );
    }
    const data: Record<string, unknown> = {};
    if (body.title_ta !== undefined)
      data.title_ta = body.title_ta?.trim() || null;
    if (body.title_en !== undefined)
      data.title_en = body.title_en?.trim() || null;
    if (body.narrative_ta !== undefined)
      data.narrative_ta = body.narrative_ta?.trim() || null;
    if (body.narrative_en !== undefined)
      data.narrative_en = body.narrative_en?.trim() || null;
    if (body.anchor_date !== undefined) {
      const d = body.anchor_date ? new Date(body.anchor_date) : null;
      if (d && Number.isNaN(d.getTime()))
        throw new BadRequestException('anchor_date invalid');
      data.anchor_date = d;
    }
    if (body.milestone_rules !== undefined)
      data.milestone_rules = body.milestone_rules as any;
    if (body.quotation_access_mode !== undefined)
      data.quotation_access_mode = validateAccessMode(
        body.quotation_access_mode,
      );
    if (body.auto_resolve_linked_complaints !== undefined)
      data.auto_resolve_linked_complaints =
        !!body.auto_resolve_linked_complaints;
    if (body.officer_self_inspection !== undefined)
      data.officer_self_inspection = !!body.officer_self_inspection;
    const updated = await this.prisma.tender.update({
      where: { id: tenderId },
      data,
    });
  ...
  async addLineItem(
    ...
    await this.audit.record({
      ...
      actorUserId,
      event: 'line_item:add',
      payload: { line_item_id: created.id },
    });
    return created;
  }
  ...
  async updateLineItem(
    ...
    if (!li || li.tender_id !== tenderId)
      throw new NotFoundException(
        `Line item #${lineItemId} not on this tender`,
      );
    const data: Record<string, unknown> = {};
    if (body.seq !== undefined) data.seq = body.seq;
    if (body.description_ta !== undefined)
      data.description_ta = body.description_ta?.trim() || null;
    if (body.description_en !== undefined)
      data.description_en = body.description_en?.trim() || null;
    if (body.quantity !== undefined)
      data.quantity = body.quantity != null ? String(body.quantity) : null;
    ...
    const updated = await this.prisma.tenderLineItem.update({
      where: { id: lineItemId },
      data,
    });
    await this.audit.record({
  ...
  async publish(orgUnitId: number, tenderId: number, actorUserId: number) {
    ...
    const lineItemCount = await this.prisma.tenderLineItem.count({
      where: { tender_id: tenderId },
    });
    if (lineItemCount === 0)
      throw new BadRequestException(
    ...
    if (t.quotation_access_mode === 'invited_only') {
      ...
      if (inviteCount === 0) {
        throw new BadRequestException(
          'Invited-only tenders need at least one vendor invite',
        );
      }
    }
  ...
  async award(
    ...
    const q = await this.prisma.tenderQuotation.findUnique({
      where: { id: quotationId },
    });
    if (!q || q.tender_id !== tenderId)
      throw new BadRequestException(
        `Quotation #${quotationId} not on this tender`,
      );
    if (q.superseded_by_id != null)
      throw new BadRequestException('Cannot award a superseded quotation');
    const updated = await this.prisma.tender.update({
      where: { id: tenderId },
      data: {
  ...
  async recordWorkCompletion(
    ...
    const completedAt = body.work_completed_at
      ? new Date(body.work_completed_at)
      : new Date();
    if (Number.isNaN(completedAt.getTime()))
      throw new BadRequestException('work_completed_at invalid');
    const updated = await this.prisma.tender.update({
      where: { id: tenderId },
      data: {
        ...
        status:
          t.status === 'vendor_selected' ? 'field_verification' : t.status,
      },
    });
    await this.audit.record({
      tenderId,
  ...
  async recordPayment(
    ...
    if (
      ...
      throw new BadRequestException(
        'Tender must be at or after vendor_selected to record payment',
      );
    }
    ...
    const meta = {
      ...((t.payment_meta as Record<string, unknown> | null) ?? {}),
      ...body.payment_meta,
    };
  ...
  private async nextVoucherSerial(orgUnitId: number): Promise<string> {
    ...
    const yearStart =
      month >= 4 ? now.getUTCFullYear() : now.getUTCFullYear() - 1;
