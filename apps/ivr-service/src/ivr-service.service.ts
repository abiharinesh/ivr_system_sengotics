...
@Injectable()
export class IvrServiceService {
    private readonly logger = new Logger(IvrServiceService.name)
    ...
    async handleServiceSelection(data: IvrCallbackDto): Promise<{ success: boolean }> {
        ...
        try {
            await this.prisma.callsMaster.upsert({
                ...
                create: {
                    call_sid: callSid, caller_number: data.CallFrom ?? data.From,
                    call_to: data.CallTo ?? data.To, flow_id: data.flow_id, tenant_id: data.tenant_id,
                ...
                update: { service_selected: !!cleanDigits, updated_at: new Date() }
            })
    ...
    async handlePollInput(data: IvrCallbackDto): Promise<PollInputResult> {
        ...
        try {
            await this.prisma.callsMaster.upsert({
                ...
                update: { poll_entered: !!cleanDigits, updated_at: new Date() }
            })
            await this.prisma.ivrPollInput.create({
                data: { call_sid: callSid, caller_number: data.CallFrom ?? data.From, poll_id: cleanDigits, raw_payload: data as any }
            })
        } catch (err) {
        ...
        try {
            serviceSelection = await this.prisma.ivrServiceSelection.findFirst({
                where: { call_sid: callSid }, orderBy: { received_at: 'desc' }
            })
        } catch (err) { this.logger.error(`[EP2] Service lookup failed: ${(err as Error).message}`) }
        ...
        const panchayat = await this.prisma.panchayat.findFirst({ where: { ivr_number: callTo } })
        if (!panchayat) {
        ...
        try {
            const complaint = await this.createServiceComplaint(serviceType, cleanDigits, panchayat.id, data.CallFrom ?? data.From ?? 'unknown')
    ...
    private async createServiceComplaint(serviceType: string, detail: string, panchayatId: number, callerNumber: string): Promise<{ id: number }> {
        if (serviceType === 'street_light') {
            const pole = await this.prisma.electricPole.findFirst({ where: { panchayat_id: panchayatId, keypad_id: detail } })
            if (!pole) {
                this.logger.warn(`[EP2] No pole found with keypad_id="${detail}" in panchayat ${panchayatId}`)
                throw new Error(`Pole with keypad_id="${detail}" not found`)
            }
            const complaint = await this.prisma.complaint.create({
                data: { pole_id: pole.id, panchayat_id: panchayatId, complaint_type: 'street_light', description: `IVR street light complaint for pole ${pole.pole_number} (keypad: ${detail}) from ${callerNumber}`, status: 'pending' }
            })
            this.logger.log(`[EP2] ✅ Street light complaint #${complaint.id} created for pole ${pole.pole_number}`)
            return complaint
        }
        const complaint = await this.prisma.complaint.create({
            data: { panchayat_id: panchayatId, complaint_type: serviceType, description: `IVR ${serviceType} complaint from ${callerNumber} (input: ${detail})`, status: 'pending' }
        })
