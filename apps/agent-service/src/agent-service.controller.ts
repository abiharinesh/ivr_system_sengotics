...
import { normalizeLandmarksField, parseGeoFromBody, strFieldOptional, JwtAuthGuard, RolesGuard, Roles } from '@app/shared'
...
export class AgentServiceController {
    ...
    createPoleWithOptionalImage(@Req() req: AuthReq, @UploadedFile() file: UploadedImageFile | undefined, @Body() body: Record<string, unknown>) {
        const panchayatId = requirePanchayatId(req)
        const landmarks = normalizeLandmarksField(body)
        if (file?.buffer?.length) {
            const geo = parseGeoFromBody(body)
            return this.agentService.createPoleWithImage(panchayatId, req.user.id, { pole_number: strFieldOptional(body, 'pole_number'), keypad_id: strFieldOptional(body, 'keypad_id'), landmarks: landmarks.length ? landmarks : undefined }, file, geo)
        }
        let latitude: number | undefined; let longitude: number | undefined
        const latStr = strFieldOptional(body, 'latitude'); const lngStr = strFieldOptional(body, 'longitude')
        if (latStr !== undefined || lngStr !== undefined) {
            ...
            if (!Number.isFinite(latitude!) || !Number.isFinite(longitude!)) throw new BadRequestException('Invalid latitude or longitude')
        }
        if ((latitude !== undefined) !== (longitude !== undefined)) throw new BadRequestException('Provide both latitude and longitude, or neither')
        return this.agentService.createPole(panchayatId, req.user.id, { pole_number: strFieldOptional(body, 'pole_number'), keypad_id: strFieldOptional(body, 'keypad_id'), latitude, longitude, landmarks: landmarks.length ? landmarks : undefined })
    }

    @Post('poles/:id/image')
    @UseInterceptors(FileInterceptor('file', { limits: { fileSize: IMAGE_LIMIT } }))
    uploadPoleImage(@Req() req: AuthReq, @Param('id', ParseIntPipe) id: number, @UploadedFile() file: UploadedImageFile, @Body() body: Record<string, unknown>) {
