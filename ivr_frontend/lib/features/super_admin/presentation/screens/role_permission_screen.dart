...
class _RolePermissionScreenState extends State<RolePermissionScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      ...
      final results = await Future.wait([rolesF, groupsF, permsF, usersF, panchayatsF]);
      ...
      setState(() {
        ...
        if (widget.preselectedUserId != null) {
          final found = _users.where((u) => u.id == widget.preselectedUserId).firstOrNull;
          if (found != null) {
            _selectedUser = found;
  ...
  Future<void> _loadUserRoles(int userId) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load user roles: $e'), backgroundColor: AppTheme.error),
      );
    } finally {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.admin_panel_settings_rounded), text: 'System Roles'),
            Tab(icon: Icon(Icons.security_rounded), text: 'Permission Groups'),
            Tab(icon: Icon(Icons.person_add_rounded), text: 'User Role Assignments'),
          ],
        ),
      ),
      body: _isLoading
          ? const AppLoadingState(message: 'Loading Roles & Permissions...', style: AppLoadingStyle.list)
          : _error != null
              ? Center(
                  child: Column(
          ...
          return Card(
            ...
            child: ListTile(
              ...
              leading: CircleAvatar(
                backgroundColor: isSystem ? AppTheme.warning.withValues(alpha: 0.15) : AppTheme.primary.withValues(alpha: 0.15),
                child: Icon(
              ...
              title: Row(
                children: [
                  Text(r['display_name'] ?? r['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  if (r['display_name_ta'] != null) ...[
                    const SizedBox(width: 8),
                    Text('(${r['display_name_ta']})', style: TextStyle(fontSize: 12, color: AppTheme.textMuted)),
                  ],
                  const SizedBox(width: 8),
                  if (isSystem)
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                      decoration: BoxDecoration(color: AppTheme.warning.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)),
                      child: const Text('System', style: TextStyle(color: AppTheme.warning, fontSize: 10, fontWeight: FontWeight.bold)),
                    ),
                ],
              ),
              subtitle: Padding(
                ...
                child: Row(
                  children: [
                    Chip(
                      label: Text('Level ${r['hierarchy_level'] ?? 5}', style: const TextStyle(fontSize: 11)),
                    ...
                    if (r['department'] != null)
                      Chip(
                        ...
                        label: Text(r['department'], style: const TextStyle(fontSize: 11)),
                        visualDensity: VisualDensity.compact,
                      ),
                  ],
                ),
  ...
  Widget _buildPermissionGroupsTab() {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('New Permission Group', style: TextStyle(color: Colors.white)),
      ),
      body: ListView.builder(
        padding: const EdgeInsets.all(16),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  ElevatedButton.icon(
                    onPressed: _selectedUser == null ? null : _showAssignRoleDialog,
          ...
          const Text('Assigned Roles for User:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Expanded(
                ...
                : _userRoles.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final roleMatch = _roles.where((r) => r['id'] == roleId).firstOrNull;
                          final roleName = roleMatch?['display_name'] ?? 'Role #$roleId';
                          final branchId = ur['branch_id'];
                          final panchayatMatch = _panchayats.where((p) => p.id == branchId).firstOrNull;
                          final branchName = panchayatMatch?.name ?? 'Branch #$branchId';
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: dispCtrl, decoration: const InputDecoration(labelText: 'Display Name (e.g. Municipal Engineer)')),
              TextField(controller: dispTaCtrl, decoration: const InputDecoration(labelText: 'Tamil Display Name (Optional)')),
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Role Key Code (e.g. municipal_engineer)')),
              TextField(controller: deptCtrl, decoration: const InputDecoration(labelText: 'Department (e.g. Engineering)')),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final key = nameCtrl.text.isNotEmpty ? nameCtrl.text : dispCtrl.text.toLowerCase().replaceAll(' ', '_');
              await _repo.createRole({
                'name': key,
                'display_name': dispCtrl.text,
                'display_name_ta': dispTaCtrl.text.isNotEmpty ? dispTaCtrl.text : null,
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              TextField(controller: dispTaCtrl, decoration: const InputDecoration(labelText: 'Tamil Display Name')),
  ...
  Future<void> _deleteRole(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot delete role: $e'), backgroundColor: AppTheme.error));
    }
  }
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDlgState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              children: [
                TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Group Name (e.g. Revenue Full Access)')),
                const SizedBox(height: 12),
                  ...
                  child: ListView(
                    children: grouped.entries.map((e) {
                      ...
                      return ExpansionTile(
                        title: Text(moduleName.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold)),
                        children: perms.map((p) {
                          final code = p['code'].toString();
                          final desc = p['description']?.toString() ?? code;
                          return CheckboxListTile(
                            title: Text(code, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                            subtitle: Text(desc, style: const TextStyle(fontSize: 11)),
                            value: selectedPerms.contains(code),
                            onChanged: (val) {
                              setDlgState(() {
                                if (val == true) selectedPerms.add(code);
                                else selectedPerms.remove(code);
                              });
                            },
                          );
                        }).toList(),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                await _repo.createPermissionGroup(name: nameCtrl.text, permissions: selectedPerms.toList());
                Navigator.pop(ctx);
                _loadAllData();
              },
  ...
  void _showEditGroupDialog(dynamic group) {
    ...
    final Map<String, dynamic> grouped = _permissionsData['grouped'] ?? {};
    final Set<String> selectedPerms = Set<String>.from(group['permissions'] ?? []);
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDlgState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                await _repo.updatePermissionGroup(group['id'], name: nameCtrl.text, permissions: selectedPerms.toList());
                Navigator.pop(ctx);
                _loadAllData();
              },
  ...
  Future<void> _deleteGroup(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot delete group: $e'), backgroundColor: AppTheme.error));
    }
  }
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Assignment failed: $e'), backgroundColor: AppTheme.error));
                }
              },
  ...
  Future<void> _revokeUserRole(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to revoke role: $e'), backgroundColor: AppTheme.error));
    }
  }
}
